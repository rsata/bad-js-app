  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (uploadedFileType === undefined) {
      ...
      if (uploadedFileType !== null && utils.startsWith(uploadedFileType.mime, 'image')) {
        ...
        if (loggedInUser) {
          fs.open(`frontend/dist/frontend/assets/public/images/uploads/1234`, 'w', function (err, fd) {
          ...
          UserModel.findByPk(loggedInUser.data.id).then(async (user: UserModel | null) => {
            if (user) {
              return await user.update({ profileImage: `assets/public/images/uploads/1234` })
            }
          }).catch((error: Error) => {
            next(error)
          })
